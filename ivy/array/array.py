...
class Array(
    ...
    def _init(self, data):
        ...
        self._size = (
            functools.reduce(mul, data.shape) if len(data.shape) > 0 else 0
        )
        self._dtype = ivy.dtype(data)
        ...
        self._is_variable = ivy.is_variable(data)
        # this _data attribute MUST be set last, due to some constrains with the graph
        #  compiler
        self._data = data
